...
class Trainer():
    ...
    def _loss_function(self, data, recon_data, latent_dist):
        ...
        if self.model.is_continuous:
            ...
            alpha, logbeta = latent_dist['cont']  # 修改的代码
            kl_cont_loss = self._kl_laplace_loss(alpha, logbeta)
            # Linearly increase capacity of continuous channels
            cont_min, cont_max, cont_num_iters, cont_gamma = \
                self.cont_capacity
            # Increase continuous capacity without exceeding disc_max
            ...
            #cont_capacity_loss = cont_gamma * torch.abs(cont_cap_current - kl_cont_loss) # 原来的代码
            cont_capacity_loss = cont_gamma * torch.abs(- kl_cont_loss) # 修改的代码
        ...
        if self.model.is_discrete:
            ...
            #disc_capacity_loss = disc_gamma * torch.abs(disc_cap_current - kl_disc_loss) # 原来的代码
            disc_capacity_loss = 0 # 修改的代码

        # Calculate total kl value to record it
        #kl_loss = kl_cont_loss + kl_disc_loss  # 原来的代码
        kl_loss = kl_cont_loss  # 修改的代码

        # Calculate total loss
        #total_loss = recon_loss + cont_capacity_loss + disc_capacity_loss  # 原来的代码
        total_loss = recon_loss + cont_capacity_loss  # 修改的代码
    ...
    def _kl_laplace_loss(self, alpha, logbeta):
        """
        ...
        # Calculate KL divergence
        #n = len(alpha)   # 修改的代码
        #kl_values = -1.0 * (1 + logbeta*0.5 - ((logbeta/2).exp())*(-alpha.abs()/((logbeta/2).exp())).exp()- alpha.abs())
        beta = torch.div(torch.exp(logbeta*0.5), 2**(0.5))  # 修改的代码
        p1 = -torch.log(beta) -1.0 # 修改的代码
        p2 = torch.mul(beta,torch.exp(-torch.div(torch.abs(alpha),beta))) # 修改的代码
        p3 = torch.abs(alpha) # 修改的代码
        kl_values = p1 + p2 + p3
        #latent_kl = (p1 + p2 +p3).mean(dim=0) # 修改的代码
        # Mean KL divergence across batch for each latent variable
        kl_means = torch.mean(kl_values, dim=0)
        # KL loss is sum of mean KL of each latent variable
        kl_loss = torch.sum(kl_means)  # 修改的代码
